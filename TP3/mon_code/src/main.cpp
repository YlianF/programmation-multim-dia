...
int main() {
	...
	sol.Couleur(RVB(0, 0.0, 0.0));
	sol.Reflexion(0.5);
	sol.setTexture(new Bitmap(&sol, (char*)"Textures/parquet.ppm",5.0f,0,0));
	lisobj.Ajouter(&sol);

	// Le plan vertical : le mur
	Plan mur;
	mur.depuisVetP(Vecteur3D(0.2, 0, 1), Point3D(0, 0, -10));
	mur.Couleur(RVB(0, 0, 0));
	mur.Ambiante(0.7);
	mur.Reflexion(0.1);
	...
	spher3.Reflexion(0.5);
	spher3.Milieu_int(1.5);
	lisobj.Ajouter(&spher3);

	Camera cam;
	cam.Centre(Point3D(0, 10, 12));
	cam.Dir(Point3D(0, -1, 0));
	cam.Haut(Vecteur3D(-0.5, 1, 0));
